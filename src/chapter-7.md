...
7.4 is solution-focused, considering the nuts-and-bolts of _how_ we might, using these insights begin to tackle the obstacles of 7.3 in pursuit of the HDR objectives. In 7.4.1 I introduce the concept of _Theories of Change (ToC)_, which are used as a framing device for structuring the four approaches to pursuing change that follow in 7.4.2 to 7.4.5. Each for these four approach sections is modelled as a different possible trajectories for change. Within each of these four trajectories, specific named opportunities are described or referenced in varying detail [^16].
...
[TODO add links to both the page, and the sitra report, and [@bowyer2022hestia; @pidoux2022]]
...
[^21]: The word _'diaspora'_ is typically used with reference to populations, but is an apt term, derived from the Greek _'diaspeirein'_ meaning _'scattered about'_ or '_dispersed'_.
...
- _**behavioural influence**_: persuading others to carry out the desired behaviour (e.g. restricting features to motivate subscription payments)
...
| The above model shows that the accumulation of data (and hence, information) is implicitly and objectively a form of power. This theory is consistent with participants’ observations in [5.4.4.1](#5.4.4.1) that data holding and limiting access to it is a source of power. We can therefore predict that as long as current platforms and service providers are free to collect so much personal information, the information landscape will remain imbalanced and individuals will not be able to acquire ecosystem negotiability.|
...
| ![Figure X: A contact-and-calendar centric PDV approach](./src/figs/figX-calendar-contact-centric-PDS-strategy.png) |
...
| ![Figure X: The Scattered Data Relating to a Vacation](./src/figs/figX-vacation.png) |
| ![Figure X: Mockup of a Unified Interface for a Vacation](./src/figs/figX-holiday-interface.jpg) |
...
The first approach to HDR reform embraces the _activist_ aspect of being a recursive public [[7.2.5](#7.2.5)], as well as the idea of reconfiguring one's world. The approach, which would be applied typically to a single service provider, app or platform (which could also be a public sector service) is fourfold:
...
collective legal action -> building a case > Uber netherlands abusing GDPR: https://medium.com/personaldata-io/uber-vs-drivers-trial-interview-data-protection-expert-rene-mahieu-55359f8cdd9d  (kinda like unionisation - reference)
examples: OUTWORLD REF noyb.eu, open rights group,
...
### Approach 2 to Improving HDR: Building the Human-Centric Future
...
### Approach 3 to Improving HDR: Defending Autonomy and Nurturing the Information Landscape
...
### Approach 4 to Improving HDR: Winning Hearts and Minds: Teaching, Championing and Selling the Vision
